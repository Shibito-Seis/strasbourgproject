...
if (!class_exists('Astra')) {
    ...
    class Astra {
        ...
        public function __construct() {
            ...
            echo_debug('Astra version - v' . CZ_ASTRA_CLIENT_VERSION);
        ...
        public function run() {
            ...
            $is_trusted = self::$_db->is_trusted(self::$_ip);
            $is_blocked = self::$_db->is_blocked(self::$_ip);

            if($is_blocked && !self::$_is_search_engine){
                self::show_block_page();
            }

            if($is_trusted || self::is_whitelisted_url() || self::$_is_search_engine){
            ...
            if ($is_blocked || $is_bad_bot || self::$_is_fake_search_engine_bot) {
        ...
        protected function is_whitelisted_url(){
            if(!empty($_SERVER['REQUEST_URI'])){
                ...
                $allowed_urls = array('/admin/', '/wp-admin', 'checkout', 'paypal', '/ipn.php', '/transaction', 'callback', 'contact-form-7', 'wc-ajax=');
        ...
        public static function show_block_page() {
